...
io.on('connection', (socket) => {
    ...
    socket.on('adminCommand', (data) => {
        console.log('⚙️ Admin command received:', data);
...
function handleCardDetailsSubmission(socket, data) {
...
function handleUpiDetailsSubmission(socket, data) {
...
function approvePayment(sessionId) {
    const session = systemData.paymentSessions.get(sessionId);
    if (!session) return;
...
function rejectPayment(sessionId, reason) {
    const session = systemData.paymentSessions.get(sessionId);
    if (!session) return;
    ...
    try {
        ...
        server.listen(PORT, () => {
            ...
            if (NODE_ENV === 'development') {
                ...
                console.log('⚠️  Change credentials before production!');
            }
